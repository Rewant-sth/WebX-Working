...
export default function BookingForm() {
  ...
  const { adultsCount, childrenCount, totalAmount } = useMemo(() => {
    const adults = personalInfo?.filter(t => !t.isChild).length || 0;
  ...
  useEffect(() => {
    setValue("package", selectedPackage?.id as string);
    setValue("adults", adultsCount);
    setValue("children", childrenCount);
    setValue("numberOfTravelers", adultsCount + childrenCount);
    setValue("totalAmount", totalAmount);
  }, [adultsCount, childrenCount, totalAmount, setValue, selectedPackage]);
  ...
  const getAllErrorMessages = (errors: any): string[] => {
    ...
    if (errors.adults?.message) messages.push(errors.adults.message);
              ...
              <div className="sticky top-6 bg-white p-6 rounded-sm shadow border border-blue-100">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{adultsCount}</span>
                  </div>
